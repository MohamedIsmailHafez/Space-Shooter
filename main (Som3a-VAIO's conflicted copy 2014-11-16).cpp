...
void DebugShip(float locX, float locY);
void Laser();
bool fireLaser;
...
GLuint playerTex, explosionTex, enemyTexI, enemyTexII;
...
public:
	...
	void Render()
	{
		for(unsigned int i=0; i<entities.size(); i++)
		{
			...
			//e->position += e->direction * Speed * e->type;
...
void DebugShip(float locX, float locY)
{
	...
	glBegin(GL_LINE_LOOP);
		glVertex2f(locX, locY + 2.0f*size);
		glVertex2f(locX + size, locY - size);
		glVertex2f(locX - size, locY - size);
	glEnd();
}
...
// Ship
//=======================================================================
void Ship(int locX, int locY, float rotation)
{
	glLoadIdentity();
	...
	if(!GameOver)
	{
		if(DEBUG)
			...
			texManager.DrawTextured(WIDTH/2, HEIGHT/2, 64, 64, playerTex);

	}
	else
	{
		texManager.DrawTextured(WIDTH/2, HEIGHT/2, 512, 512, explosionTex);
		GameState = GAMEOVER;
	}
}
...
void CalculateFPS()
{
	...
	if (_time - _timebase > 1000) {
		sprintf(s,"FPS = %1.0f", frame*1000.0/(_time-_timebase));
		_timebase = _time;
		frame = 0;
	}
}
...
{
	glPushMatrix();
	renderText(s, 5, glutGet(GLUT_WINDOW_HEIGHT)-50, 2);
	glPopMatrix();
}
...
void Display(void)
{
	...
	//elapsedTimeLastFrame =  currentTime - lastFrame;
...
void LoadTextures()
{
	playerTex		= texManager.LoadTexture("Alpha.bmp", true);
